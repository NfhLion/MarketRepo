...
void ChatMain::SetChatList()
{
    ...
    QString str_line1 = "总部";
    QString str_line2 = "连锁店";
    QString str_line3 = "顾客";
    ui->treeWidget_friend->insertGroupItem(line, str_line1);
    ui->treeWidget_friend->insertGroupItem(line + 1, str_line2);
    ui->treeWidget_friend->insertGroupItem(line + 2, str_line3);  
}
...
void ChatMain::SlotInsertUserYes(const int line, const QString &id, const QString &name, const QString &type)
{
    ...
    if(type == "market"){
        ...
        ui->treeWidget_friend->addChildItem(ui->treeWidget_friend->topLevelItem(line_tree + 2), chitem);
    }
    //给每个好友列表设置定时器进行有好友发消息时通知
    QTimer *msgTimer = new QTimer(this);
    ...
    json.insert("mtype", QString("shop"));
    json.insert("qid", id);
    QJsonDocument doc(json);
    QByteArray data = doc.toJson();
    MsgSocket::GetMsgSocket()->SendMessage(data);
